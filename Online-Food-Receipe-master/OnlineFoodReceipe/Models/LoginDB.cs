...
namespace OnlineFoodReceipe.Models
{
    public class LoginDB
    {
        ...
        public List<Feedback> FeedbackInfo()
        {
            ...
            string str = "select Top 3 * from Feedback order by Email desc";
            cmd=new SqlCommand(str, con);
            con.Open();
        ...
        {
            List<Feedback> list = new List<Feedback>();
            string str = "select * from Feedback order by Email desc";
            cmd = new SqlCommand(str, con);
            con.Open();
            dr = cmd.ExecuteReader();
            if (dr.HasRows)
        ...
        {
            string str = "insert into Login(Username,Email,Password,RoleID,PhotoName) values(@name,@email,@password,@roleid,@photo)";
            cmd = new SqlCommand(str, con);
            con.Open();
        ...
        {
            string str = "insert into Logged(Name,Password) values(@email,@password)";
            cmd = new SqlCommand(str, con);
            con.Open();
            cmd.Parameters.AddWithValue("@email", Email);
        ...
        {
            string str = "update Logged set Vnb=@vnb where Name=@email";
            cmd = new SqlCommand(str, con);
            con.Open();
            cmd.Parameters.AddWithValue("@email", Email);
        ...
        {
            string str = "update Logged set Sname=@sname where Name=@email";
            cmd = new SqlCommand(str, con);
            con.Open();
            cmd.Parameters.AddWithValue("@email", Email);
